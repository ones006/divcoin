...
bool CryptoNoteProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA& hshd, CryptoNoteConnectionContext& context, bool is_inital) {
  ...
  if (context.m_state == CryptoNoteConnectionContext::state_synchronizing) {
    ...
    logger(diff >= 0 ? (is_inital ? Logging::INFO : Logging::DEBUGGING) : Logging::TRACE, Logging::BRIGHT_GREEN) << context <<
      "Your DivCoin node is syncing with the network. You are "
...
bool CryptoNoteProtocolHandler::request_missing_objects(CryptoNoteConnectionContext& context, bool check_having_blocks) {
  if (context.m_needed_objects.size()) {
    ...
    context.m_state = CryptoNoteConnectionContext::state_normal;
    logger(Logging::INFO, Logging::BRIGHT_GREEN) << context << "Successfully synchronized with the DivCoin Network.";
    on_connection_synchronized();
  }
  return true;
}
...
bool CryptoNoteProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    logger(Logging::INFO)
      << ENDL ;
      logger(INFO, BRIGHT_MAGENTA) << "===[ DivCoin Tip! ]=============================" << ENDL ;
      logger(INFO, WHITE) << " Always exit DivCoind and zedwallet with the \"exit\" command to preserve your chain and wallet data." << ENDL ;
      ...
      logger(INFO, WHITE) << " If you need more assistance, visit the #HELP channel in the DivCoin Discord Chat - http://chat.turtlecoin.lol" << ENDL ;
      logger(INFO, BRIGHT_MAGENTA) << "===================================================" << ENDL << ENDL ;

      logger(INFO, BRIGHT_GREEN) <<

      #ifdef _WIN32
      "\n______   _____   _   _   _____   _____   _____   _   _ \n"
        "|  _  \ |_   _| | | | | /  __ \ |  _  | |_   _| | \ | |\n"
        "| | | |   | |   | | | | | /  \/ | | | |   | |   |  \| |\n"
        "| | | |   | |   | | | | | |     | | | |   | |   | . ` |\n"
        "| |/ /   _| |_  \ \_/ / | \__/\ \ \_/ /  _| |_  | |\  |\n"
